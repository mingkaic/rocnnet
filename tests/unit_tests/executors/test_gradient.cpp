...
TEST(DERIVE, unary) {
	...
	for (size_t i = 0; i < len; i++)
	{
		...
		grad->collect_grad([&count, &raw](ivariable<T>* key, placeholder<T>* value)
		{
...
TEST(DERIVE, binary) {
	...
	std::vector<double> ra = nnet::expose<double>(a)->get_raw();
	std::vector<double> rb = nnet::expose<double>(b)->get_raw();
	...
	for (size_t i = 0; i < univars.size(); i++) {
		...
		grad->collect_grad([&count, &raw1, &raw2](ivariable<T>* key, placeholder<T>* value)
		{
		    if (0 == count)
		...
		ASSERT_EQ(2, count);
		count = 0;
		bad_grad->collect_grad([&count](ivariable<T>* key, placeholder<T>* value)
		{
		    count++;
		});
		ASSERT_EQ(0, count);
