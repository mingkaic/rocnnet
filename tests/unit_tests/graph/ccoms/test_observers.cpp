...
TEST(CCOMS, AttachAndDetach)
{
	...
	EXPECT_CALL(*subject, attach()).Times(3);
	EXPECT_CALL(*subject, detach()).Times(3);
...
TEST(CCOMS, NotifyAndUpdate)
{
	...
	EXPECT_CALL(o1, update()).Times(1);
	EXPECT_CALL(o2, update()).Times(1);
	EXPECT_CALL(o3, update()).Times(1);
...
TEST(CCOMS, LeafCollection)
{
	...
	mock_subject leaf4;
	mock_subject leaf5;
	EXPECT_CALL(leaf1, merge_leaves);
	EXPECT_CALL(leaf2, merge_leaves).Times(2);
	EXPECT_CALL(leaf3, merge_leaves);
	EXPECT_CALL(leaf4, merge_leaves);
	EXPECT_CALL(leaf5, merge_leaves);
	
	mock_observer branch1(leaf1, leaf2);
	mock_observer branch2(leaf4);
	mock_observer branch3(branch1, leaf3);
	mock_observer branch4(leaf2, leaf5);
	mock_observer branch5(branch3, branch4); // test for set join
