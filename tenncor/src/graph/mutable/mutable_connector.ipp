...
template <typename T>
void mutable_connector<T>::update (ccoms::caller_info info, ccoms::update_message msg)
{
	if (ccoms::update_message::REMOVE_ARG == msg.cmd_)
	{
		disconnect();
	}
	else
...
template <typename T>
bool mutable_connector<T>::add_arg (ivariable<T>* var, size_t idx)
{
