...
TEST(ALLOCATOR, Allocate_A001)
{
	...
	size_t numa = FUZZ<size_t>::get(1, {1, 127})[0];
	size_t numb = FUZZ<size_t>::get(1, {1, 127})[0];
...
TEST(ALLOCATOR, Deallocate_A002)
{
	...
	size_t numa = FUZZ<size_t>::get(1, {1, 127})[0];
	size_t numb = FUZZ<size_t>::get(1, {1, 127})[0];
	size_t numc = FUZZ<size_t>::get(1, {25, 127})[0];
	...
	while (i < numc)
	{
		...
		if (i < numc-1)
		{
			...
			if (potential > 5)
			{
				size = FUZZ<size_t>::get(1, {1, potential/2})[0];
			}
		}
