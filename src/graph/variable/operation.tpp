...
tensor<T>* compress_op (const tensor<T>& in, size_t index) {
	// allocate dimension 1 along index
	// (if index is begining or end of shape, then remove dimension)
	tensor_shape ts = in.get_shape();
	return nullptr;
}
...
template <typename T>
void iunar_mat_ops<T>::shape_eval (void) {
...
template <typename T>
extend<T>::extend (ivariable<T>& in) {
	(*this)(in);
}
...
tensor<T>* compress<T>::calc_derive (ivariable<T>* over) const {
	...
	if (this->var) {
		...
		if (deriv) {
			...
			ans = this->compress_op(*deriv, index);
			delete deriv;
		}
	}
	return ans;
...
template <typename T>
compress<T>::compress (ivariable<T>& in) {
	(*this)(in);
}

template <typename T>
compress<T>::compress (ivariable<T>& in, size_t index) : index(index) {
	(*this)(in);
}

template <typename T>
void compress<T>::set_cmpr_info (size_t index) {
	this->index = index;
}
...
const tensor<T>& compress<T>::eval (void) {
	assert(nullptr != this->var);
	const tensor<T>& in = this->var->eval();
	tensor<T>* ans = this->compress_op(in, index);
	this->out = *ans;
	delete ans;
	return this->out;
}
